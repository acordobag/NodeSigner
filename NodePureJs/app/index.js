...
async function main() {
    ...
    // Read cert
    // const certPem = fs.readFileSync("cert.p12", { encoding: "utf8" });
    // const certDer = pem2der(certPem);

    // Read key
    const keyPem = fs.readFileSync("cert.p12", { encoding: "utf8" });
    const keyDer = pem2der(keyPem);
    console.log(keyDer)
    const key = await crypto.subtle.importKey("jwk", keyDer, alg, false, ["sign"]);
